...
| `region/internal/`           | Unpublished, unauthenticated Region typeahead and lookup (states, counties, ZIPs) over the ES `regions` alias, for ServiceNet's Geography filter — see [docs/internal-regions.md](docs/internal-regions.md) and, for how the four internal routes fit together, [docs/geography-filter.md](docs/geography-filter.md). |
