...
| `region/internal/`           | Unpublished, unauthenticated Region typeahead and lookup (states, counties, ZIPs) over the ES `regions` alias, for ServiceNet's Geography filter — see [docs/internal-regions.md](docs/internal-regions.md). |
...
| `service/`                   | Service detail by id (MongoDB). `service/internal` holds the unpublished, unauthenticated services search and facets routes ServiceNet's record selector uses — see [docs/internal-services-search.md](docs/internal-services-search.md). |
