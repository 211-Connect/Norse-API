...
| Module                       | Purpose                                                                                                                                                                                               |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `search/`                    | Resource search (`SearchService`/`HybridSearchService`) + AI classification/re-rank proxy (`AiSearchService`) to ML Broker. See [docs/ai-search-classification.md](docs/ai-search-classification.md). the `organization_id` filter scopes results to one organization by its stable id — see [docs/organization-search.md](docs/organization-search.md). |
| `resource/`                  | Single-resource lookups against Elasticsearch. `rank` on phone numbers is zero-based while `rank` on addresses is one-based, and the arrays are not reliably sorted — see [docs/phone-and-address-rank.md](docs/phone-and-address-rank.md) before reading either positionally. |
...
| `organization/`              | Organization lookups (typeahead search + detail). See [docs/organization-search.md](docs/organization-search.md) and, for the read-only `SERVICE_AT_LOCATIONS[].DISPLAY` view, [docs/service-at-location-display.md](docs/service-at-location-display.md). |
