...
| Module                       | Purpose                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `search/`                    | Resource search (`SearchService`/`HybridSearchService`) + AI classification/re-rank proxy (`AiSearchService`) to ML Broker. See [docs/ai-search-classification.md](docs/ai-search-classification.md). the `organization_id` filter scopes results to one organization by its stable id — see [docs/organization-search.md](docs/organization-search.md). The opt-in `relevance_cutoff` param trims low-relevance hybrid results — see [docs/relevance-cutoff.md](docs/relevance-cutoff.md). |
...
| `organization/`              | Organization lookups (typeahead search + detail). See [docs/organization-search.md](docs/organization-search.md).                                                                                                                                                                                                                                        |
...
| `region/internal/`           | Unpublished, internal-key-guarded (not tenant-scoped) Region typeahead and lookup (states, counties, ZIPs) over the ES `regions` alias, for ServiceNet's Geography filter — see [docs/internal-regions.md](docs/internal-regions.md) and, for how the four internal routes fit together, [docs/geography-filter.md](docs/geography-filter.md). Other modules import only its public surface, `region/internal/index.ts` (`REGIONS_INDEX`, `REGION_ID_PATTERN`, `regionIdMessage`, `RegionService.assertExist` via `RegionServiceModule`). |
| `suggestion/`                | Search/autocomplete suggestions — always returns combined taxonomy+organization typeahead results in one round trip (delegates the taxonomy half to `TaxonomyService`) — see [docs/organization-search.md](docs/organization-search.md).                                                                                                                 |
| `service/`                   | Service detail by id (MongoDB). `service/internal` holds the unpublished, internal-key-guarded (not tenant-scoped) services search and facets routes ServiceNet's record selector uses — see [docs/internal-services-search.md](docs/internal-services-search.md) and [docs/geography-filter.md](docs/geography-filter.md). |
...
| `common/`                    | Cross-cutting: config, guards, middleware, filters, decorators, Mongoose schemas, request-scoped cache service. `common/elasticsearch/es-call.ts` is the shared ES error mapping (HttpException passes, timeout `503`, else `502`, optional per-call mapper). `common/testing/` holds test-only read-only ES/Mongo clients.                                   |
| `live/`                      | **Test-only.** Live stress specs against a real Elasticsearch (and optionally Mongo), skipped unless `NORSE_LIVE_ES=1` — see [docs/geography-filter.md](docs/geography-filter.md#live-test-against-a-real-cluster). Like `common/testing/`, excluded from `nest build` by `tsconfig.build.json`; `src/common/testing/build-exclusion.spec.ts` enforces it. |
...
- A route that is genuinely not tenant-scoped opts out of the global `TenantScopeGuard`
  with `@NotTenantScoped()` ([src/auth/gateway/gateway-principal.ts](src/auth/gateway/gateway-principal.ts))
  and must carry another guard in its place — the Geography filter routes use
  `InternalApiGuard`. It exempts from tenant scope only; gateway identity and permissions still run.
- Don't log secrets (`ML_BROKER_API_KEY`, Keycloak tokens, etc.).
...
- Test-only helpers live in `src/common/testing/` or `src/live/`, both excluded from
  the production build. Production code must not import from them (the
  build-exclusion spec fails if it does); add any new test-only directory to
  `tsconfig.build.json` `exclude` and to that spec.
