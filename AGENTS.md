...
| `organization/`              | Organization lookups (typeahead search + detail). See [docs/organization-search.md](docs/organization-search.md). The detail response's per-service `SERVICE_AT_LOCATIONS[].DISPLAY` — the resolved, read-only view a seeker sees for one service at one location — is documented in [docs/service-at-location-display.md](docs/service-at-location-display.md). |
