...
- Validation migration note: for any **new** request/response DTO classes, use `class-validator` (+ `class-transformer` when needed) and DTO-driven Swagger decorators. Avoid introducing new Zod schemas for new features; keep Zod only where already in place until migration is finished.
